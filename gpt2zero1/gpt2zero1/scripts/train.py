...
import gpt2zero1.core.model as model_lib
import gpt2zero1.core.config as config
import gpt2zero1.data.load as load
import gpt2zero1.data.save as save
import gpt2zero1.data.sharding as sharding
...
def compute_updated_params(grads, params, device_optimizer_state):
    device_id = jax.lax.axis_index("device")

    grads = sharding.split_pytree(grads, config.gpt2_config.num_devices)
    params = sharding.split_pytree(params, config.gpt2_config.num_devices)

    grads_fns = [lambda split=split: split for split in grads]
    params_fns = [lambda split=split: split for split in params]

    device_grads = jax.lax.switch(device_id, grads_fns)
...
def update_step(params, optimizer_state, batch):
    ...
    optimizer_out_specs = jax.tree_util.tree_map(
        ...
        optimizer_state,  # Use the structure of the input optimizer_state as reference
    )
